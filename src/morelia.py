...
class ReportVisitor:
    ...
    def visit(self, node):
        ...
        recon, we_owe =  node.to_html(error=error)
...
class Feature(Morelia):
    ...
    def to_html(self, error=False):
        color = '#ff0000' if error else '#aaffbb'
        return ['\n<div><table><tr style="background-color: {0};" width="100%">'
                                  '<td align="right" valign="top" width="100">'
                                  '<em>{1}</em>:</td><td colspan="101">{2}</td></tr></table></div>'.format(color, self.concept,  _clean_html(self.predicate)), '']
...
class Scenario(Morelia):
    ...
    def to_html(self, error=False):
        color = '#ff0000' if error else '#cdffb8'
        return ["""
        <div><table width="100%"><tr style="background-color: {0};">
        <td align="right" valign="top" width="100"><em>{1}</em>:</td>
        <td colspan="101">{2}</td></tr>""".format(color, self.concept, _clean_html(self.predicate)),
        '</table></div>']
...
# ERGO  use "born again pagan" somewhere
    ...
    def to_html(self, error=False):
        highlight = 'style="background-color: red;"' if error else ''
        return ('''
        <tr><td align="right" valign="top" {0}><em>{1}</em></td>
        <td colspan="101">{2}</td></tr>'''.format(highlight, self.concept,
            _clean_html(self.predicate)), '')
